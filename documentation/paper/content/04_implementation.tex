...
Storage alternative that is deployable on mobile devices such as smart phones and tablets, since users hold their own claims.
\item \label{db_req_two}
Free licensing structure for development.
\item \label{db_item_three}
Lightweight storage for user side claim management.\ref{db_req_one}
\item \label{db_req_four}
Schemaless files since claim structure can vary by claim type and value. Removing the need for strict typography of data attributes as used in RDBMS.
...
Afterwards we checked the various types of data storage possibilities and rated them according to our criteria.
...
Couchbase lite is deployable on all modern operating systems including but not limited to iOS, Android, Windows.
...
One of couchbases particularities is the bucket design. A bucket is meant quite literally, as in you can put whatever data you want into it and save it until changed or deleted. Couchbase itself also offers very comfortable retrieval and updating mechanisms by
offering a built-in upsert method. Which either retrieves an entity and overwrites it with a new verison or creates a new document from the given document.
